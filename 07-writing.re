...
DTP(DeskTop Publishing)と呼ばれるものが一般化した現代、ひとまず執筆環境としてはコンピュータを使ったものを前提としてこの章では色々紹介してみます。

身もふたもないことを言ってしまうと、どんなツールを使おうと、PDFが出力できればなんら問題ありません。
...
また、複数人で書く場合、GoogleDocなどを使うのも有効です。オンラインで共同編集可能ですし、環境構築不要で、印刷したときの見た目にかなり近い執筆環境を手に入れることができます。複数人で作業するときも、全く同じ部分を同時編集することは少ないので、意図しない変更や競合が起こることは少なく、余り気にする必要はありません。

== テキストファイルをコンパイルする系の版組ツールを使う

なんらかのテキストエディタを使って書いた”ソースコード”をコンパイルして成果物を得るというのはソフトウェア技術者にとってはなじみ深い手順です。技術同人誌の多くはそういった手法が使われています。

  * Re:VIEW
  * Markdown
  * HTML+CSS(Vivliostyleなど)
  * Sphinx
  * TeX/LaTeX
...
技術同人誌界隈でよく使われるのは、技術同人誌の大家であるTechBoosterさんがそのノウハウを惜しみなく詰め込んだリポジトリを公開しているため、環境さえ構築できれば、そのまま書き始められるという利点があるためです。
...
実は、本書の同人誌バージョンもGitHubですべて公開されています。https://github.com/onestop-techbook/c93-onestop-techbookというリポジトリが、本書の同人誌バージョンのソースコードなのです。環境を整えれば、同人誌版のPDFやepubを生成できます。本書の底本は、@<tt>{ReVIEW-Template}をベースに書かれています。今日ではだいぶ調整していますが…
...
なお、Re:VIEWの欠点は、バージョンによって挙動が異なったり、コンパイル時のエラーがわかりづらかったり、といったこと。そして、TeXに関する闇がいろいろあるところです。また、ちょっと標準テンプレートト違うことをやろうと思ったとき、いきなりTeXとRe:VIEWそのものと戦う必要が出てきます。
...
実際いつの間にか、”//blankline” という機能が追加されました。2018年頃、Re:VIEW Ver2.5 で実装されたようです。したがって、2017年発行の最初の同人版では、全角スペースというBadKnowHowを使っていましたが・・・

こういうBadKnowHowだって記録しておくことに意味があるでしょうから、残しておきましょう。
====[/column]
...
=== Vivliostyle（HTML/Markdown+CSS組版)
...
MarkdownまたはHTMLとCSSからPDFやEPUBを出力することができ、適切なテンプレートを使えば印刷可能なPDFを生成することができます。

Vivliostyle CLIはコマンドラインからPDFを書き出すことができ、また豊富な作成例があります。

Vivliostyleユーザー会のみなさんが「Vivliostyleで本を作ろう」というシリーズでVivliostyleを使った本を定期的に発行しています。なんならそのテンプレートをそのまま使えばいきなり印刷用のPDFを生成することができますね！
...
おっと、TeXも忘れちゃいけません。Re:VIEWはTeXのフロントエンドであり、裏ではTeXが走っていますが、一方で本家TeXはお作法はだいぶ複雑です。情報系、数学系、工学系の人は一度は触ったことがあるかと思いますが…使い慣れないとなかなか難しいようです。自由度も高く、数式は確かにきれいに書けますが、@<em>{これを期にTeXを始めよう、というのはあまりおすすめできません。}書き始めたものの、途中でハマってしまい、肝心の執筆が止まってしまうというリスクがあります。もちろん、ある程度TeXに慣れている人にとっては、いきなりTeXで書くことも十分に選択肢として考慮してよいでしょう。

なお、数式を多用する場合、しかも微分積分、あるいは累乗や添字が飛び交うような文章、その他の数学記号が攻めてくるような文章を書く場合は選択肢に入るでしょう。

なお、TeXのフロントエンドとして、LyX@<fn>{LyX}を使うというのは良い選択です。TeXのおまじないがかなり軽減されていて、ワープロソフト／DTPソフトとしてかなり使い勝手が良くなります。
//footnote[LyX][@<href>{https://www.lyx.org/WebJa.Home}]

====[column] 執筆環境構築からはじめてはいけない
本を書こうと思ったとき、何から始めますか？

ネタを探す。目次を考える。いいですね。

やってはいけないこと。それは、@<em>{最初に執筆環境の構築をする}です。

執筆環境の構築を始めると、それにはまってしまい、本文を書く時間が無くなります。または、執筆環境の構築に満足してしまい、謎のやり遂げた感によって本文執筆のモチベーションが霧散してしまいます。

大事なことなのでもう一度言います。執筆環境構築からはじめてはいけません。
====[/column]
...
この本はGithubとRe:VIEWを主軸として執筆しています。エンジニアに馴染みやすく多人数での執筆に向いています@<fn>{solo}。技術書典で出展されている同人誌ではよく見られる構成です。まずは執筆環境の概要を説明します。
...
DockerはWindows環境だと癖がある、と長らく言われていましたが、昨今(少なくとも2023年現在)大幅に改善しました。またMacやLinux環境なら動かしやすいでしょう。そして、Dockerの利点は、クラウドのサービスで動かしやすいというのもあります。ローカルの環境とクラウドの環境を同一、シームレスにすることも可能です。次に説明するWercerCIはまさにその性質を活用しています。



=== Wercker

注）werckerは2022年10月末をもってサービス終了しました。他のCIサービスにおいても大差はないでしょうから、本書の記述は残しますが、他のサービスを使うためには、必要に応じて適宜読み替えてください。
...
中でもVisual Studio Code（VSCode）は名前の通り Visual Studio なので、テキストエディタというよりは IDE なのですが、とても軽量でテキストエディタとしてもとても使い勝手が良く、最近シェアを急激に伸ばしています。実際、本書や別の本も、Re:VIEW プラグインをインストールした VSCode で書いています。筆者が試した限りだと Atom より動作が軽量かつ、JavaScript/TypeScript での IDE としての機能が筆者にとっての必要十分なので、愛用しているエディタです。
...
//footnote[atom][@<href>{https://atom.io/}　GitHubが開発していたオープンソースのテキストエディタ。ただ、最近は親会社のVSCodeに押され、2022年末で開発終了しました。]
...
ここまで、本を書くツールおよびその書き方について説明しました。本章で述べた細かい動きは、環境やバージョンによって異なることがあります。頑張ってください、というエールを送りつつ、目的を見失わないように頑張ってください。あなたの目標は何ですか？Dockerを使いこなせるようになることですか？Re:VIEWで文章を書けるようになることですか？違いますよね？

技術同人誌を作ることですよね。

そのことを忘れないようにして、本を作り上げましょう。

決して、環境構築して満足したりしないように・・・
...
結論から言ってしまえば、Wordでも普通に本は書けるので、環境構築で挫折しそうになるくらいなら、(Dockerを使ったことのない方、特にWindowsユーザーは)とっととWord等のワープロソフトで書いてしまいましょう。WordはWordで癖があるソフトですが、業務用PC、あるいは家庭用PCにもほぼ標準的にインストールされていることもあり、WindowsユーザーでWordを一度も使ったことがないという人はかなりまれかと思います。ということは、とっかかりの操作方法に悩む必要がないソフトだといえます。また、比較的編集中の見た目と印刷結果が近いので、直感的な執筆/組版に向きます。Re:VIEWやMarkdown、TeXなどは、本文と制御コードの羅列ですから、出来上がりを想像することはなかなか大変です。またInDesignなどはお作法がかなり厳しかったり、操作を覚えてまともに書けるようになるまでが結構たいへんです。

書き始めるためのハードルはできるだけ下げて、とにかく最初の本を書き上げることを優先すべきです。その上で「Wordだダメだ！」と思ったら、次の本で他の執筆環境を試せばよいのです。いきなり使ったことないツールを使い始めて、そのツールの使い方習得に忙殺され、本が完成しないというのは本末転倒です。執筆ツールはあくまで執筆ツールです。（Text:おやかた）
